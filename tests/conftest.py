...
Also wires module-import-time coverage so child Python
interpreters (subprocess.run, jupyter nbconvert, etc.)
report their coverage data. `os.environ.setdefault`
would be a no-op here because pytest-cov has already
set COVERAGE_FILE to a tmp dir by the time conftest is
loaded — force-set is required.
"""
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_str_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
